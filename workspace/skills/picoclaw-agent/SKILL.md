---
name: picoclaw-agent
description: "Configure, extend, debug, or contribute to PicoClawQuant itself. Use when the task is about PicoClawQuant CLI commands, config.json, gateway, auth, models, skills, MCP servers, cron, routing, sessions, self-evolution, built-in slash commands, or repository internals. Use PicoClawQuant-native workflows, terminology, paths, and configuration."
metadata: {"nanobot":{"emoji":"🦞"}}
---

# PicoClawQuant Agent

PicoClawQuant is a lightweight personal AI assistant and agent framework with a native CLI, chat gateway, MCP integration, installable skills, session routing, scheduled jobs, and self-evolution.

Use this skill when the job is about **PicoClawQuant itself**: onboarding, configuration, debugging, adding features, extending the CLI, changing routing/session behavior, working on skills or MCP support, or contributing to this repository.

## Operating Stance

When this skill is active, stay fully subordinate to PicoClawQuant's real architecture:

- Prefer PicoClawQuant commands, config keys, workspace layout, and docs.
- Follow PicoClawQuant source and docs for behavior, naming, and workflows.
...
PicoClawQuant supports 30+ LLM providers through `model_list`.

Credential patterns in PicoClawQuant are:
...
| Provider | `provider` value | Auth path in PicoClawQuant |
| --- | --- | --- |
...
PicoClawQuant also carries first-class metadata or routing support for additional vendors such as:
...
PicoClawQuant tools are configured under `tools` in `config.json` and registered dynamically at runtime.
...
PicoClawQuant has a first-class subagent model for long-running work, isolated subproblems, and multi-agent specialization.
...
When an agent has the `spawn` tool and at least one allowed peer, PicoClawQuant injects a lightweight agent registry into the system prompt automatically.
...
```text
spawn(
  task="Search the web for the latest PicoClawQuant MCP integration patterns and summarize them.",
  label="mcp-research",
  agent_id="research"
)
```
...
PicoClawQuant can transcribe inbound audio and synthesize outbound speech, but voice setup is model-driven like the rest of the runtime.
...
| ElevenLabs Scribe | `provider: elevenlabs`, `model: scribe_v1` | Uses PicoClawQuant's dedicated ElevenLabs transcription path |
...
- if it resolves to an ElevenLabs model, PicoClawQuant uses the ElevenLabs transcriber
- if it resolves to a Whisper-compatible model, PicoClawQuant uses the Whisper transcription path
- if it resolves to an audio-capable multimodal model, PicoClawQuant can use audio-model transcription
- if `voice.model_name` is omitted, PicoClawQuant still performs compatibility scanning across `model_list` for legacy auto-detected ASR entries
...
| --- | --- | --- |
| OpenAI-compatible speech | `openai/tts-1` | Best-supported path; PicoClawQuant sends an OpenAI-style `/audio/speech` request |
...
- if that is missing, PicoClawQuant can still scan `model_list` for the first API-backed model whose ID contains `tts`
...
- if a provider rejects `response_format`, PicoClawQuant retries once without that field
- `send_tts` is only registered when TTS detection succeeds


## In-Session Slash Commands

PicoClawQuant's shared slash command registry lives under `pkg/commands`.
...
PicoClawQuant is model-centric. The key fields are:
...
- If `provider` is set, PicoClawQuant sends `model` to that provider unchanged.
...
PicoClawQuant only relies on `name` and `description` frontmatter fields for loading and matching.

### MCP Discovery

PicoClawQuant supports always-loaded and deferred MCP tools.
...
Start with the most PicoClawQuant-native path:
...
For gateway and runtime debugging, PicoClawQuant writes logs under its home directory:
...
- the `provider` and `model` fields use PicoClawQuant's model-centric rules
