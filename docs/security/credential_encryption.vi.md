...
PicoClawQuant hỗ trợ mã hóa các giá trị `api_key` trong các mục cấu hình `model_list`.
...
Nếu `PICOCLAW_SSH_KEY_PATH` không được đặt, PicoClawQuant tìm khóa chuyên dụng:
