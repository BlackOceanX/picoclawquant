...
PicoClawQuant supports separating sensitive data (API keys, tokens, secrets, passwords) from the main configuration by storing them in a `.security.yml` file. This improves security by:
...
Restart PicoClawQuant and verify it loads correctly:
```bash
picoclaw --version
```
...
PicoClawQuant supports encrypting API keys in the security file for additional protection.
