...
PicoClawQuant supports encrypting `api_key`/`api_keys` values in `model_list` configuration entries.
...
If `PICOCLAW_SSH_KEY_PATH` is not set, PicoClawQuant looks for the picoclaw-specific key:
