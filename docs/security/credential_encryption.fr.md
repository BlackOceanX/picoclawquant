...
PicoClawQuant prend en charge le chiffrement des valeurs `api_key` dans les entrées de configuration `model_list`.
...
Exécutez `picoclawquant onboard` — il vous demande votre phrase secrète et génère la clé SSH,
...
Si `PICOCLAW_SSH_KEY_PATH` n'est pas défini, PicoClawQuant recherche la clé dédiée :
...
Exécutez `picoclawquant onboard` pour le générer automatiquement.
...
> **Remarque :** Un fichier de clé SSH est requis pour le chiffrement des identifiants. Si aucune clé n'est trouvée et que `PICOCLAW_SSH_KEY_PATH` n'est pas défini, le chiffrement/déchiffrement échouera. Exécutez `picoclawquant onboard` pour générer la clé automatiquement.
...
- **La phrase secrète et la clé SSH sont toutes deux requises.** La clé SSH agit comme un second facteur — sans elle, le chiffrement/déchiffrement échouera. Exécutez `picoclawquant onboard` pour générer la clé si elle n'existe pas.
- **La clé SSH est en lecture seule à l'exécution.** PicoClawQuant n'écrit ni ne modifie jamais le fichier de clé SSH.
