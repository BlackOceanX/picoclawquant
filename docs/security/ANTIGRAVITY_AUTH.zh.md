...
**Antigravity**（Google Cloud Code Assist）是由 Google 支持的 AI 模型提供商，通过 Google 的云基础设施提供对 Claude Opus 4.6 和 Gemini 等模型的访问。本文档提供了关于认证工作原理、如何获取模型以及如何在 PicoClawQuant 中实现新提供商的完整指南。
...
8. [API 端点](#api-端点)
9. [配置](#配置)
10. [在 PicoClawQuant 中创建新提供商](#在-picoclaw-中创建新提供商)
...
- Go ≥ 1.25
- PicoClawQuant 代码库（`pkg/providers/` 和 `pkg/auth/`）
...
## 在 PicoClawQuant 中创建新提供商

PicoClawQuant 提供商以 Go 包的形式实现，位于 `pkg/providers/` 下。要添加新提供商：
