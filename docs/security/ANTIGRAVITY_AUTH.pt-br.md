...
**Antigravity** (Google Cloud Code Assist) é um provedor de modelos de IA apoiado pelo Google que oferece acesso a modelos como Claude Opus 4.6 e Gemini através da infraestrutura de nuvem do Google. Este documento fornece um guia completo sobre como a autenticação funciona, como buscar modelos e como implementar um novo provedor no PicoClawQuant.
...
10. [Criando um Novo Provedor no PicoClawQuant](#criando-um-novo-provedor-no-picoclaw)
...
- Go ≥ 1.25
- Base de código do PicoClawQuant (`pkg/providers/` e `pkg/auth/`)
...
## Criando um Novo Provedor no PicoClawQuant

Os provedores do PicoClawQuant são implementados como pacotes Go em `pkg/providers/`. Para adicionar um novo provedor:
