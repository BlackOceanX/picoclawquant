...
<h1>PicoClawQuant: 基于Go语言的超高效 AI 助手</h1>
...
<p>
  ...
  创建、预览并分享面向 PicoClawQuant 生态的 AI 桌面伙伴。
</p>
...
> **PicoClawQuant** 是由 [矽速科技 (Sipeed)](https://sipeed.com) 发起的独立开源项目，完全使用 **Go 语言**从零编写——不是 OpenClaw、NanoBot 或其他项目的分支。

🦐 **PicoClawQuant** 是一个受 [NanoBot](https://github.com/HKUDS/nanobot) 启发的超轻量级个人 AI 助手。它采用 **Go 语言** 从零重构，经历了一个"自举"过程——即由 AI Agent 自身驱动了整个架构迁移和代码优化。
...
> **🚨 安全声明**
>
> - **无加密货币 (NO CRYPTO):** PicoClawQuant **没有** 发行任何官方代币、Token 或虚拟货币。所有在 `pump.fun` 或其他交易平台上的相关声称均为 **诈骗**。
> - **官方域名:** 唯一的 PicoClawQuant 官方网站是 **[picoclaw.io](https://picoclaw.io)**，公司官网是 **[sipeed.com](https://sipeed.com)**。
> - **警惕:** 许多相似的 `.ai/.org/.com/.net/...` 后缀域名被第三方抢注。请只信任本 README 明确链接的域名。
> - **注意:** PicoClawQuant 正在初期的快速功能开发阶段，可能有尚未修复的网络安全问题，在 1.0 正式版发布前，请不要将其部署到生产环境中。
> - **注意:** PicoClawQuant 最近合并了大量 PR，近期版本可能内存占用较大 (10~20MB)，我们将在功能较为收敛后进行资源占用优化。

## 📢 新闻

2026-06-11 🐾 **PicoPaw AI 上线！** 访问 [picopaw.ai/zh](https://picopaw.ai/zh)，体验全新的 PicoPaw 桌面伙伴，预览会动的 AI 宠物，并关注 PicoClawQuant 生态更新。

2026-05-11 🛒 **LicheeRV-Claw 已上架淘宝！** 现在可以在 [淘宝](https://item.taobao.com/item.htm?abbucket=20&id=764939520376) 购买 LicheeRV-Claw，更方便地在小型 RISC-V 硬件上体验 PicoClawQuant。
...
2026-03-31 📱 **Android 支持！** PicoClawQuant 现可在 Android 上运行！APK 下载地址：[picoclaw.io](https://picoclaw.io/download)

2026-03-25 🚀 **v0.2.4 发布！** Agent 架构全面重构（SubTurn、Hook、Steering、EventBus）、微信/企业微信深度集成、安全体系升级（.security.yml、敏感数据过滤）、新增 Provider（AWS Bedrock、Azure、小米 MiMo），以及 35 项 Bug 修复。PicoClawQuant 已达 **26K ⭐**！

2026-03-17 🚀 **v0.2.3 发布！** 系统托盘 UI（Windows & Linux）、子 Agent 状态查询 (`spawn_status`)、实验性 Gateway 热重载、Cron 安全门控，以及 2 项安全修复。PicoClawQuant 已达 **25K ⭐**！
...
2026-02-26 🎉 PicoClawQuant 仅 17 天突破 **20K Stars**！频道自动编排和能力接口上线。

2026-02-16 🎉 PicoClawQuant 一周内突破 12K Stars！社区维护者角色和 [路线图](../../ROADMAP.md) 正式发布。

2026-02-13 🎉 PicoClawQuant 4 天内突破 5000 Stars！项目路线图和开发者群组筹建中。

2026-02-09 🎉 **PicoClawQuant 正式发布！** 仅用 1 天构建，将 AI Agent 带入 $10 硬件与 <10MB 内存的世界。🦐 皮皮虾，我们走！
...
PicoClawQuant 几乎可以部署在任何 Linux 设备上！
...
也可以从 [GitHub Releases](https://github.com/BlackOceanX/picoclawquant/releases) 页面手动下载对应平台的二进制文件。
...
> **Docker / 虚拟机用户：** Gateway 默认监听 `127.0.0.1`。设置 `PICOCLAWQUANT_GATEWAY_HOST=0.0.0.0` 或使用 `-public` 参数以允许从宿主机访问。
...
pkg install proot
termux-chroot ./picoclawquant onboard   # chroot 提供标准 Linux 文件系统布局
```
...
PicoClawQuant 通过 `model_list` 配置支持 30+ LLM Provider，使用 `协议/模型` 格式：
...
通过 18+ 消息平台与你的 PicoClawQuant 对话：
...
PicoClawQuant 可以搜索网络以提供最新信息。在 `tools.web` 中配置：
...
PicoClawQuant 内置文件操作、代码执行、定时任务等工具。详情请参阅 [工具配置](../reference/tools_configuration.zh.md)。
...
PicoClawQuant 原生支持 [MCP](https://modelcontextprotocol.io/) — 连接任意 MCP 服务器，通过外部工具和数据源扩展 Agent 能力。
...
通过 CLI 或任何已集成的聊天应用发送一条消息，即可将 PicoClawQuant 连接到 Agent 社交网络。
...
| 命令                       | 说明                   |
| ------------------------- | ---------------------- |
| `picoclawquant onboard`        | 初始化配置与工作区       |
| `picoclaw auth weixin` | 扫码连接微信个人号 |
| `picoclawquant agent -m "..."` | 与 Agent 对话           |
| `picoclawquant agent`          | 交互式对话模式           |
...
PicoClawQuant 通过 `cron` 工具支持定时提醒和重复任务：
...
查看完整的 [社区路线图](https://github.com/BlackOceanX/picoclawquant/issues/988) 和 [CONTRIBUTING.md](../../CONTRIBUTING.md)。
