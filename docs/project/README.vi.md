...
<h1>PicoClawQuant: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>

<h3>Phần cứng $10 · RAM 10MB · Khởi động ms · Let's Go, PicoClawQuant!</h3>
  <p>
...
> **PicoClawQuant** là một dự án mã nguồn mở độc lập do [Sipeed](https://sipeed.com) khởi xướng, được viết hoàn toàn bằng **Go** từ đầu — không phải fork của OpenClaw, NanoBot hay bất kỳ dự án nào khác.

**PicoClawQuant** là trợ lý AI cá nhân siêu nhẹ lấy cảm hứng từ [NanoBot](https://github.com/HKUDS/nanobot). Nó được xây dựng lại từ đầu bằng **Go** thông qua quá trình "tự khởi động" — chính AI Agent đã dẫn dắt quá trình di chuyển kiến trúc và tối ưu hóa mã nguồn.
...
> **Thông báo Bảo mật**
>
> * **KHÔNG CÓ CRYPTO:** PicoClawQuant **chưa** phát hành bất kỳ token hay tiền điện tử chính thức nào. Mọi thông tin trên `pump.fun` hoặc các nền tảng giao dịch khác đều là **lừa đảo**.
...
> * **LƯU Ý:** PicoClawQuant đang trong giai đoạn phát triển nhanh. Có thể còn các vấn đề bảo mật chưa được giải quyết. Không triển khai lên môi trường production trước v1.0.
> * **LƯU Ý:** PicoClawQuant gần đây đã merge nhiều PR. Các bản build gần đây có thể dùng 10-20MB RAM. Tối ưu hóa tài nguyên được lên kế hoạch sau khi tính năng ổn định.
...
2026-05-11 🛒 **LicheeRV-Claw đã có trên AliExpress!** Bạn hiện có thể mua LicheeRV-Claw trên [AliExpress](https://www.aliexpress.com/item/1005006519668532.html), giúp việc thử PicoClawQuant trên phần cứng RISC-V nhỏ gọn dễ dàng hơn.
...
2026-03-31 📱 **Hỗ trợ Android!** PicoClawQuant giờ chạy trên Android! Tải APK tại [picoclaw.io](https://picoclaw.io/download)

2026-03-25 🚀 **v0.2.4 đã phát hành!** Tái cấu trúc kiến trúc Agent (SubTurn, Hooks, Steering, EventBus), tích hợp WeChat/WeCom, tăng cường bảo mật (.security.yml, lọc dữ liệu nhạy cảm), provider mới (AWS Bedrock, Azure, Xiaomi MiMo) và 35 bản vá lỗi. PicoClawQuant đã đạt **26K Stars**!

2026-03-17 🚀 **v0.2.3 đã phát hành!** Giao diện system tray (Windows & Linux), truy vấn trạng thái sub-agent (`spawn_status`), thử nghiệm Gateway hot-reload, bảo mật Cron, và 2 bản vá bảo mật. PicoClawQuant đã đạt **25K Stars**!
...
2026-02-26 🎉 PicoClawQuant đạt **20K Stars** chỉ trong 17 ngày! Tự động điều phối Channel và giao diện khả năng đã hoạt động.

2026-02-16 🎉 PicoClawQuant vượt 12K Stars trong một tuần! Vai trò người duy trì cộng đồng và [Lộ trình](../../ROADMAP.md) chính thức ra mắt.

2026-02-13 🎉 PicoClawQuant vượt 5000 Stars trong 4 ngày! Lộ trình dự án và nhóm nhà phát triển đang được xây dựng.

2026-02-09 🎉 **PicoClawQuant ra mắt!** Được xây dựng trong 1 ngày để đưa AI Agent lên phần cứng $10 với <10MB RAM. Let's Go, PicoClawQuant!
...
PicoClawQuant có thể được triển khai trên hầu hết mọi thiết bị Linux!
...
Ngoài ra, tải binary cho nền tảng của bạn từ trang [GitHub Releases](https://github.com/BlackOceanX/picoclawquant/releases).
...
> **Người dùng Docker / VM:** Gateway lắng nghe trên `127.0.0.1` theo mặc định. Đặt `PICOCLAWQUANT_GATEWAY_HOST=0.0.0.0` hoặc dùng cờ `-public` để có thể truy cập từ host.
...
Hãy cho chiếc điện thoại cũ của bạn một cuộc sống mới! Biến nó thành Trợ lý AI thông minh với PicoClawQuant.
...
PicoClawQuant hỗ trợ 30+ Provider LLM thông qua cấu hình `model_list`. Sử dụng định dạng `protocol/model`:
...
Trò chuyện với PicoClawQuant của bạn qua 17+ nền tảng nhắn tin:
...
PicoClawQuant có thể tìm kiếm web để cung cấp thông tin cập nhật. Cấu hình trong `tools.web`:
...
PicoClawQuant bao gồm các tool tích hợp sẵn cho thao tác tệp, thực thi mã, lên lịch và nhiều hơn nữa. Xem [Cấu hình Tools](../reference/tools_configuration.vi.md) để biết chi tiết.
...
PicoClawQuant hỗ trợ [MCP](https://modelcontextprotocol.io/) gốc — kết nối bất kỳ MCP server nào để mở rộng khả năng Agent của bạn với các tool và nguồn dữ liệu bên ngoài.
...
Kết nối PicoClawQuant với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn duy nhất qua CLI hoặc bất kỳ Ứng dụng Chat nào đã tích hợp.
...
| Lệnh                      | Mô tả                                    |
| ------------------------- | ---------------------------------------- |
| `picoclawquant onboard`        | Khởi tạo cấu hình & workspace           |
| `picoclaw auth weixin` | Kết nối tài khoản WeChat qua QR |
| `picoclawquant agent -m "..."` | Trò chuyện với agent                     |
| `picoclawquant agent`          | Chế độ trò chuyện tương tác             |
...
PicoClawQuant hỗ trợ nhắc nhở đã lên lịch và tác vụ định kỳ thông qua tool `cron`:
...
Xem [Lộ trình Cộng đồng](https://github.com/BlackOceanX/picoclawquant/issues/988) và [CONTRIBUTING.md](../../CONTRIBUTING.md) để biết hướng dẫn.
