...
<h1>PicoClawQuant: Assistente de IA Ultra-Eficiente em Go</h1>

<h3>Hardware de $10 · 10MB de RAM · Boot em ms · Let's Go, PicoClawQuant!</h3>
  <p>
...
> **PicoClawQuant** é um projeto open-source independente iniciado pela [Sipeed](https://sipeed.com), escrito inteiramente em **Go** do zero — não é um fork do OpenClaw, NanoBot ou qualquer outro projeto.

**PicoClawQuant** é um assistente de IA pessoal ultra-leve inspirado no [NanoBot](https://github.com/HKUDS/nanobot). Foi reconstruído do zero em **Go** por meio de um processo de "auto-bootstrapping" — o próprio AI Agent conduziu a migração de arquitetura e a otimização do código.
...
> **Aviso de Segurança**
>
> * **SEM CRIPTO:** O PicoClawQuant **não** emitiu nenhum token oficial ou criptomoeda. Todas as alegações no `pump.fun` ou outras plataformas de negociação são **golpes**.
...
> * **NOTA:** O PicoClawQuant está em desenvolvimento rápido inicial. Podem existir problemas de segurança não resolvidos. Não implante em produção antes da v1.0.
> * **NOTA:** O PicoClawQuant mesclou muitos PRs recentemente. Builds recentes podem usar 10-20MB de RAM. A otimização de recursos está planejada após a estabilização de funcionalidades.
...
2026-05-11 🛒 **LicheeRV-Claw no AliExpress!** Agora você pode comprar o LicheeRV-Claw no [AliExpress](https://www.aliexpress.com/item/1005006519668532.html), facilitando testar o PicoClawQuant em hardware RISC-V compacto.
...
2026-03-31 📱 **Suporte Android!** PicoClawQuant agora roda no Android! Baixe o APK em [picoclaw.io](https://picoclaw.io/download)

2026-03-25 🚀 **v0.2.4 Lançada!** Reformulação da arquitetura Agent (SubTurn, Hooks, Steering, EventBus), integração WeChat/WeCom, fortalecimento de segurança (.security.yml, filtragem de dados sensíveis), novos providers (AWS Bedrock, Azure, Xiaomi MiMo) e 35 correções de bugs. O PicoClawQuant atingiu **26K Stars**!

2026-03-17 🚀 **v0.2.3 Lançada!** UI na bandeja do sistema (Windows e Linux), consulta de status de sub-agent (`spawn_status`), hot-reload experimental do Gateway, controle de segurança do Cron e 2 correções de segurança. O PicoClawQuant atingiu **25K Stars**!
...
2026-02-26 🎉 O PicoClawQuant atinge **20K Stars** em apenas 17 dias! Orquestração automática de channels e interfaces de capacidade estão disponíveis.

2026-02-16 🎉 O PicoClawQuant ultrapassa 12K Stars em uma semana! Funções de mantenedor da comunidade e [Roadmap](../../ROADMAP.md) lançados oficialmente.

2026-02-13 🎉 O PicoClawQuant ultrapassa 5000 Stars em 4 dias! Roadmap do projeto e grupos de desenvolvedores em andamento.

2026-02-09 🎉 **PicoClawQuant Lançado!** Construído em 1 dia para levar AI Agents a hardware de $10 com menos de 10MB de RAM. Let's Go, PicoClawQuant!
...
O PicoClawQuant pode ser implantado em praticamente qualquer dispositivo Linux!
...
Alternativamente, baixe o binário para sua plataforma na página de [GitHub Releases](https://github.com/BlackOceanX/picoclawquant/releases).
...
> **Usuários de Docker / VM:** O Gateway escuta em `127.0.0.1` por padrão. Defina `PICOCLAWQUANT_GATEWAY_HOST=0.0.0.0` ou use a flag `-public` para torná-lo acessível pelo host.
...
Dê uma segunda vida ao seu celular de uma década! Transforme-o em um Assistente de IA inteligente com o PicoClawQuant.
...
pkg install proot
termux-chroot ./picoclawquant onboard   # chroot fornece um layout padrão de sistema de arquivos Linux
```
...
O PicoClawQuant suporta mais de 30 providers de LLM através da configuração `model_list`. Use o formato `protocolo/modelo`:
...
Converse com seu PicoClawQuant por meio de mais de 17 plataformas de mensagens:
...
O PicoClawQuant pode pesquisar na web para fornecer informações atualizadas. Configure em `tools.web`:
...
O PicoClawQuant inclui ferramentas integradas para operações de arquivo, execução de código, agendamento e mais. Veja [Configuração de Ferramentas](../reference/tools_configuration.pt-br.md) para detalhes.
...
O PicoClawQuant suporta nativamente o [MCP](https://modelcontextprotocol.io/) — conecte qualquer servidor MCP para estender as capacidades do seu Agent com ferramentas externas e fontes de dados.
...
Conecte o PicoClawQuant à Rede Social de Agents simplesmente enviando uma única mensagem via CLI ou qualquer App de Chat integrado.
...
| Comando                   | Descrição                              |
| ------------------------- | -------------------------------------- |
| `picoclawquant onboard`        | Inicializar config e workspace         |
| `picoclaw auth weixin` | Conectar conta WeChat via QR |
| `picoclawquant agent -m "..."` | Conversar com o agent                  |
| `picoclawquant agent`          | Modo de chat interativo                |
...
O PicoClawQuant suporta lembretes agendados e tarefas recorrentes através da ferramenta `cron`:
...
Veja nosso [Roadmap da Comunidade](https://github.com/BlackOceanX/picoclawquant/issues/988) e [CONTRIBUTING.md](../../CONTRIBUTING.md) para diretrizes.
