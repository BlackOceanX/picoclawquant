...
如果你是在开发 PicoClawQuant 本体，而不是只想验证协议，那么再看后面的 Go in-process 示例。
...
当前宿主不会接受 process hook 主动发起的新 RPC。也就是说，外部 hook 现在只能“响应 PicoClawQuant 的调用”，不能反向调用宿主去发送 channel 消息。
