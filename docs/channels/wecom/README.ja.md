...
PicoClawQuant は WeCom を公式 WeCom AI Bot WebSocket API に基づく単一の `channels.wecom` チャンネルとして公開します。
従来の `wecom`、`wecom_app`、`wecom_aibot` の分割を統一された設定モデルに置き換えました。

> パブリックな Webhook コールバック URL は不要です。PicoClawQuant は WeCom へのアウトバウンド WebSocket 接続を確立します。
...
- PicoClawQuant はアクティブな WeCom ターンを維持し、可能な限り同じストリームでストリーミング返信を継続します。
