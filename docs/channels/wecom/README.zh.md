...
PicoClawQuant 将企业微信整合为单一的 `channels.wecom` 渠道，基于腾讯官方企业微信 AI Bot WebSocket API 实现。
原有的 `wecom`、`wecom_app`、`wecom_aibot` 三个独立渠道已合并为统一配置模型。

> 本渠道无需公网 Webhook 回调地址。PicoClawQuant 主动向企业微信建立出站 WebSocket 连接。
...
- PicoClawQuant 维护活跃的企业微信 Turn，流式回复尽可能在同一流上继续。
