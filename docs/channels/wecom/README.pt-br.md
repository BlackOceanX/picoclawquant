...
O PicoClawQuant expõe o WeCom como um único canal `channels.wecom`, construído sobre a API WebSocket oficial do WeCom AI Bot.
Isso substitui a antiga separação `wecom`, `wecom_app` e `wecom_aibot` por um modelo de configuração unificado.

> Nenhuma URL de callback webhook pública é necessária. O PicoClawQuant estabelece uma conexão WebSocket de saída para o WeCom.
...
- O PicoClawQuant mantém um turno WeCom ativo para que as respostas em streaming possam continuar no mesmo fluxo quando possível.
