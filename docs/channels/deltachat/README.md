...
PicoClawQuant can run as a Delta Chat bot by launching a local
...
PicoClawQuant can find it automatically; otherwise set `rpc_server_path` to the
exact binary path.
...
The easiest setup is to let PicoClawQuant create a chatmail account in Delta
  ...
  "channel_list": {
    "deltachat": {
      ...
      "settings": {
        ...
        "display_name": "PicoClawQuant Bot",
...
On startup, PicoClawQuant creates the account through `deltachat-rpc-server`, then
stops with an error that contains the generated address. Replace the relay
marker with that full email address and run PicoClawQuant again:
...
`password` is not needed for PicoClawQuant-created chatmail accounts. Omit it when
...
for classic email accounts that PicoClawQuant must configure itself. In that mode,
...
PicoClawQuant applies them on every startup, so changing the avatar path in config is
...
| `password` | No | Legacy only; required when PicoClawQuant must configure/reconfigure a classic mailbox itself |
...
With `email` set to `@server`, PicoClawQuant creates the chatmail account, prints
the generated full email in the startup error, and exits. Update `email` to that
full address and run PicoClawQuant again. On later runs, PicoClawQuant selects the
configured account by `email`, applies optional profile settings, marks it as a
bot, and starts IO.

With a new `data_dir` plus legacy `password`, PicoClawQuant can still configure a
...
it. On startup PicoClawQuant prints the bot invite link and QR code. Add the bot from
...
| `created chatmail account ...` | Replace the `@server` marker in `email` with the generated full email and run PicoClawQuant again |
