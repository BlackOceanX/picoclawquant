...
PicoClawQuant 支持将任意 MQTT 客户端作为消息渠道。设备或服务向 Broker 发布请求，PicoClawQuant 订阅后处理并将响应发布回去。
...
| `agent_id` | PicoClawQuant 实例标识，对应配置中的 `agent_id` 字段 |
...
连接断开后 PicoClawQuant 会自动以 5 秒间隔重连 Broker，重连成功后自动重新订阅。断线重连时复用相同的 Broker 客户端 ID，Broker 能正确识别为同一连接。
...
- **client_id 与会话 ID 的区别**：topic 路径中的 `client_id` 由客户端应用自行设置，用于区分会话；它与 PicoClawQuant paho 连接 Broker 时使用的客户端 ID 是两个独立的概念。
- **多实例部署**：若多个 PicoClawQuant 实例使用相同 `agent_id` 连接同一 Broker，需为每个实例配置不同的 `client_id` 以避免 Broker 层面的冲突。
