...
PicoClawQuant prend en charge n'importe quel client MQTT comme canal de messagerie. Les appareils ou services publient des requêtes vers un broker ; PicoClawQuant s'abonne, les traite et publie les réponses en retour.
...
| `agent_id` | Identifiant de l'instance PicoClawQuant, défini dans le champ `agent_id` |
...
PicoClawQuant se reconnecte automatiquement au broker en cas de perte de connexion, avec un intervalle de 5 secondes. L'abonnement est rétabli automatiquement. L'ID client côté broker reste identique à chaque reconnexion.
...
- **client_id vs ID de session** : Le `client_id` dans le chemin du topic est défini par votre application cliente. Il est distinct de l'ID client paho utilisé par PicoClawQuant pour se connecter au broker.
- **Instances multiples** : Si plusieurs instances PicoClawQuant utilisent le même `agent_id` sur le même broker, définir des `client_id` distincts pour éviter les conflits.
