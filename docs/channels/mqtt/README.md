...
PicoClawQuant supports any MQTT client as a chat channel. Devices or services publish requests to a broker; PicoClawQuant subscribes, processes them, and publishes responses back.
...
| `agent_id` | PicoClawQuant instance identifier, set in `agent_id` config field |
...
PicoClawQuant automatically reconnects to the broker if the connection is lost, with a 5-second retry interval. On reconnect, the subscription is re-established automatically. The broker-side client ID stays the same across reconnects so the broker correctly identifies it as the same session.
...
- **client_id vs session ID**: The `client_id` in the topic path is set by your client application and identifies the conversation session. It is separate from the broker-level client ID used by PicoClawQuant's paho connection.
- **Multiple instances**: If you run multiple PicoClawQuant instances against the same broker with the same `agent_id`, set distinct `client_id` values to avoid broker-level conflicts.
