...
PicoClawQuant stores scheduled jobs in the current workspace and can run them either as reminders, full agent turns, or shell commands.

## Schedule Types

PicoClawQuant currently uses three schedule forms in the cron tool:
...
When the job fires, PicoClawQuant sends the saved message back through the agent loop as a new agent turn. Use this for scheduled work that may need reasoning, tools, or a generated reply.

### `deliver: true`

When the job fires, PicoClawQuant publishes the saved message directly to the target channel and recipient without agent processing.

The CLI `picoclaw cron add --deliver` flag uses this mode.

### `command`

When a cron-tool job includes `command`, PicoClawQuant runs that shell command through the `exec` tool and publishes the command output back to the channel.
...
This is not a hard disable switch. If you set `allow_command` to `false`, PicoClawQuant still allows a command job when the caller explicitly passes `command_confirm: true`.
...
to PicoClawQuant can schedule shell commands. Use it only when every enabled remote
