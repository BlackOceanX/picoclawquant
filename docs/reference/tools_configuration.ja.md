...
PicoClawQuant のツール設定は `config.json` の `tools` フィールドにあります。
...
デフォルトで、PicoClawQuant は以下の危険なコマンドをブロックします：
...
exec ガードは PicoClawQuant に送信されたトップレベルのコマンドのみを検証します。そのコマンドの実行開始後にビルドツールやスクリプトが生成する子プロセスを再帰的に検査することは**ありません**。
