...
A configuração de ferramentas do PicoClawQuant está localizada no campo `tools` do `config.json`.
...
Por padrão, o PicoClawQuant bloqueia os seguintes comandos perigosos:
...
O guarda exec apenas valida o comando de nível superior enviado ao PicoClawQuant. Ele **não** inspeciona recursivamente processos filhos gerados por ferramentas de build ou scripts após o início desse comando.
