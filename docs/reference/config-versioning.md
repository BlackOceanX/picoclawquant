...
PicoClawQuant uses a schema versioning system for `config.json` to ensure smooth upgrades as the configuration format evolves.
...
When you run PicoClawQuant with a V2 config file:

1. **Detection**: PicoClawQuant reads the `version` field and detects V2
...
⚠️ **Important**: Once migrated to V3, the config **cannot** be safely loaded by older PicoClawQuant versions that only support V2.

**To downgrade:**

1. Stop PicoClawQuant
2. Restore the backup:
   ...
   ```
3. Use a PicoClawQuant version that supports V2 configs
