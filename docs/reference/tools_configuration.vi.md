...
Cấu hình công cụ của PicoClawQuant nằm trong trường `tools` của `config.json`.
...
Theo mặc định, PicoClawQuant chặn các lệnh nguy hiểm sau:
...
Bộ bảo vệ exec chỉ xác thực lệnh cấp cao nhất được gửi đến PicoClawQuant. Nó **không** kiểm tra đệ quy các tiến trình con được tạo bởi các công cụ build hoặc script sau khi lệnh đó bắt đầu chạy.
