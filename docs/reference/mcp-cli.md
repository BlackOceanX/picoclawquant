...
PicoClawQuant includes an `mcp` CLI command group for managing MCP server entries in `config.json`.
...
- saves atomically
- preserves the standard 2-space JSON formatting used by PicoClawQuant
...
- use the `--` separator when the stdio command itself has arguments that may look like PicoClawQuant CLI flags
- without `--`, PicoClawQuant treats the first two non-flag tokens as `<name>` and `<command-or-url>`
...
- if `<name>` already exists, PicoClawQuant asks for confirmation
- use `--force` to skip the prompt

Local path validation:

- if the command looks like a local path such as `./server.py` or `/opt/mcp/server`
- PicoClawQuant checks that the file exists
- on non-Windows platforms, it also checks that the file is executable

Clear URL/transport error:

- if the target looks like `https://...` but transport is still `stdio`, PicoClawQuant returns an explicit error telling you to use `--transport http` or `--transport sse`
...
If the removed server was the last configured MCP server, PicoClawQuant also disables `tools.mcp.enabled`.
...
- without `--status`, PicoClawQuant prints configuration state only
- with `--status`, PicoClawQuant tries to connect to each enabled server and reports `ok (N tools)` or `error`
...
4. Start PicoClawQuant normally so the configured MCP server is loaded by the host.
