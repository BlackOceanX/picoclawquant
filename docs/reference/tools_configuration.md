...
PicoClawQuant's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, PicoClawQuant can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
If `range` is omitted, PicoClawQuant performs an unrestricted search.
...
By default, PicoClawQuant blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to PicoClawQuant. It does **not** recursively inspect child
...
If you prefer not to edit JSON manually, PicoClawQuant also provides an MCP configuration manager CLI:
