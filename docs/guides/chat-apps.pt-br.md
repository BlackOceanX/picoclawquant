...
| **Pico**             | ⭐ Fácil           | Canal de protocolo nativo PicoClawQuant                    |                                                                                                                  |
...
O PicoClawQuant agora mantém definições de comandos em um registro compartilhado. Na inicialização, o Telegram registrará automaticamente os comandos de bot suportados (por exemplo `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) para que o menu de comandos e o comportamento em tempo de execução permaneçam sincronizados.
...
Se o registro de comandos falhar (erros transitórios de rede/API), o canal ainda inicia e o PicoClawQuant tenta novamente o registro em segundo plano.
...
O PicoClawQuant pode se conectar ao WhatsApp de duas formas:
...
O PicoClawQuant suporta conexão com sua conta pessoal do WeChat usando a API oficial Tencent iLink.
...
O PicoClawQuant suporta três tipos de integração WeCom:
...
O PicoClawQuant se conecta ao Feishu via modo WebSocket/SDK — não é necessário URL de webhook público nem servidor de callback.
...
OneBot é um protocolo aberto para bots QQ. O PicoClawQuant se conecta a qualquer implementação compatível com OneBot v11 (ex.: [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
...
Qualquer cliente MQTT pode se comunicar com o PicoClawQuant via broker. Dispositivos ou serviços publicam requisições para o broker; o PicoClawQuant assina, processa e publica as respostas de volta.
...
```
{prefix}/{agent_id}/{client_id}/request    # Cliente → PicoClawQuant
{prefix}/{agent_id}/{client_id}/response   # PicoClawQuant → Cliente
```
