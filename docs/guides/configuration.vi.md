...
PicoClawQuant lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.picoclaw/workspace`):
...
`turn_profile` là chính sách tùy chọn trong `agents.defaults.turn_profile` để kiểm soát ngữ cảnh mỗi turn mới mang theo: lịch sử, system prompt, prompt skills và các tool được phép gọi. Nếu không cấu hình, hoặc đặt `"enabled": false`, PicoClawQuant giữ nguyên hành vi mặc định. Khi đặt `"enabled": true`, chính sách bên dưới áp dụng cho mỗi turn mới.

Mỗi block dùng chung các giá trị `mode`:

| Mode | Ý nghĩa |
| --- | --- |
| `default` | Giữ hành vi bình thường của PicoClawQuant. Block bị thiếu hoặc thiếu `mode` đều được xem là `default`. |
...
| `system_prompt` | Chèn identity mặc định của PicoClawQuant, chỉ dẫn workspace, memory, runtime context và summary. System prompt từ request bên ngoài vẫn được dùng khi block này `off`. |
...
Khi `system_prompt.mode` là `off`, tools vẫn hiển thị và không có system prompt bên ngoài, PicoClawQuant dùng lại quy tắc dùng tool hiện có làm prompt fallback tối thiểu. Nếu `tools.mode` là `off`, fallback này không được thêm.
...
PicoClawQuant chạy trong môi trường sandbox mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong workspace đã cấu hình.
...
Guard bảo mật exec chỉ kiểm tra dòng lệnh mà PicoClawQuant khởi chạy trực tiếp. Nó không kiểm tra đệ quy các tiến trình con được tạo bởi công cụ phát triển được phép như `make`, `go run`, `cargo`, `npm run`, hoặc script build tùy chỉnh.
...
* Chạy PicoClawQuant trong container hoặc VM nếu bạn cần cách ly mạnh hơn guard tích hợp.
...
PicoClawQuant có thể thực hiện tác vụ định kỳ tự động. Tạo file `HEARTBEAT.md` trong workspace:
...
| `volcengine` | LLM (Volcengine trực tiếp)              | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=PicoClawQuant&utm_content=PicoClawQuant&utm_medium=devrel&utm_source=OWO&utm_term=PicoClawQuant) |
...
> **Tính năng mới:** PicoClawQuant hiện sử dụng cách tiếp cận **lấy mô hình làm trung tâm**. Chỉ cần chỉ định định dạng `vendor/model` (ví dụ: `zhipu/glm-4.7`) để thêm provider mới — **không cần thay đổi code!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Lấy](https://www.volcengine.com/activity/codingplan?utm_campaign=PicoClawQuant&utm_content=PicoClawQuant&utm_medium=devrel&utm_source=OWO&utm_term=PicoClawQuant) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Chỉ OAuth                                                        |

#### Cân Bằng Tải

Cấu hình nhiều endpoint cho cùng tên mô hình — PicoClawQuant sẽ tự động round-robin:
...
Hành vi lỗi được giữ thận trọng: nếu streaming lỗi trước khi gửi bất kỳ chunk hiển thị nào, PicoClawQuant thử lại một lần qua đường dẫn `Chat()` thông thường. Nếu đã có chunk hiển thị cho người dùng, PicoClawQuant không gửi thêm một câu trả lời non-streaming thứ hai để tránh lặp output.

### Kiến Trúc Provider

PicoClawQuant định tuyến provider theo họ giao thức:
...
PicoClawQuant hỗ trợ tác vụ theo lịch qua công cụ `cron`.
