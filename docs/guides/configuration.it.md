...
Lo streaming del provider usa un double opt-in ed è disattivato per impostazione predefinita. L'agent prova lo streaming solo quando il canale corrente ha `settings.streaming.enabled: true`, l'entry del modello attivo ha `streaming.enabled: true`, e sia il provider sia il canale supportano lo streaming. Se manca una qualsiasi condizione, PicoClawQuant usa il normale percorso di richiesta non streaming.
...
Il comportamento in caso di errore è intenzionalmente conservativo: se lo streaming fallisce prima che venga inviato un chunk visibile, PicoClawQuant riprova una volta tramite il normale percorso `Chat()`. Se un chunk è già stato mostrato all'utente, PicoClawQuant non invia una seconda risposta non streaming, evitando output duplicato.

### Struttura del Workspace

PicoClawQuant salva i dati nel workspace configurato (predefinito: `~/.picoclaw/workspace`):
...
Se `tools` è omesso nel frontmatter, l'agent riceve il normale set globale dei tool abilitati. Se `tools` è presente, PicoClawQuant registra per quell'agent solo i tool runtime elencati.
...
Quando un agent ha peer spawnabili, PicoClawQuant inietta automaticamente nel suo system prompt un registry strutturato dei peer. Non serve una chiamata aggiuntiva a un tool `list_agents`.
...
PicoClawQuant esegue in un ambiente sandboxed per impostazione predefinita. L'agent può accedere solo ai file ed eseguire comandi all'interno del workspace configurato.
...
Il controllo di sicurezza exec ispeziona solo la riga di comando avviata direttamente da PicoClawQuant. Non ispeziona ricorsivamente i processi figlio generati da strumenti di sviluppo consentiti come `make`, `go run`, `cargo`, `npm run` o script di build personalizzati.
...
* Esegui PicoClawQuant in un container o VM se hai bisogno di un isolamento più forte di quello fornito dal controllo integrato.
...
PicoClawQuant può eseguire task periodici automaticamente. Crea un file `HEARTBEAT.md` nel tuo workspace:
