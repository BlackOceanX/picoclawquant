...
In PicoClawQuant, routing has two user-facing parts:
...
If no rule matches, PicoClawQuant falls back to the default agent.
...
- PicoClawQuant resolves the light model at startup; if it is invalid, routing is disabled
