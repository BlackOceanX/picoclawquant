...
This guide covers running the PicoClawQuant terminal binary on an ARM64 Android phone with Termux. Use the APK from [picoclaw.io](https://picoclaw.io/download/) if you want the Android app experience; use Termux when you want a lightweight command-line install on an older or resource-constrained device.
...
The default workspace is `~/.picoclaw/workspace`. If you want PicoClawQuant to read or write Android shared storage, run `termux-setup-storage` first and then point the workspace or any file paths at the mounted storage directory.
...
Keep the Termux session open while PicoClawQuant is running. Android battery optimization can stop background work, so disable battery optimization for Termux if you expect PicoClawQuant to keep running after the screen locks.
...
| Files or paths behave differently than Linux | Run PicoClawQuant through `termux-chroot` instead of calling the binary directly. |
| Provider requests fail | Check the API key and network access in `~/.picoclaw/config.json`. |
| PicoClawQuant stops when the phone sleeps | Disable Android battery optimization for Termux and keep a foreground Termux session active. |
