...
# PicoClawQuant で Antigravity プロバイダーを使用する

このガイドでは、PicoClawQuant で **Antigravity**（Google Cloud Code Assist）プロバイダーをセットアップして使用する方法を説明します。
...
6.  **PicoClawQuant が待機しているターミナルにそれを貼り付けます**。

PicoClawQuant が自動的に認証コードを抽出し、プロセスを完了します。
...
*   **429 レート制限**：Antigravity には厳格なクォータがあります。制限に達した場合、PicoClawQuant はエラーメッセージに「リセット時間」を表示します。
