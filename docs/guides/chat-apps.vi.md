...
| **Pico**             | ⭐ Dễ              | Kênh giao thức bản địa PicoClawQuant                       |                                                                                                                  |
...
PicoClawQuant hiện lưu trữ định nghĩa lệnh trong một registry chung. Khi khởi động, Telegram sẽ tự động đăng ký các lệnh bot được hỗ trợ (ví dụ `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) để menu lệnh và hành vi runtime luôn đồng bộ.
...
Nếu đăng ký lệnh thất bại (lỗi tạm thời mạng/API), kênh vẫn khởi động và PicoClawQuant thử lại đăng ký trong nền.
...
PicoClawQuant có thể kết nối WhatsApp theo hai cách:
...
PicoClawQuant hỗ trợ kết nối với tài khoản WeChat cá nhân của bạn thông qua API chính thức Tencent iLink.
...
PicoClawQuant hỗ trợ ba loại tích hợp WeCom:
...
PicoClawQuant kết nối với Feishu qua chế độ WebSocket/SDK — không cần URL webhook công khai hay máy chủ callback.
...
OneBot là giao thức mở cho bot QQ. PicoClawQuant kết nối với bất kỳ triển khai tương thích OneBot v11 nào (ví dụ: [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) qua WebSocket.
...
Bất kỳ client MQTT nào đều có thể giao tiếp với PicoClawQuant qua broker. Thiết bị hoặc dịch vụ publish yêu cầu lên broker; PicoClawQuant subscribe, xử lý và publish phản hồi trở lại.
