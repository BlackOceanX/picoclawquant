...
PicoClawQuant 支持多种聊天平台，使您的 Agent 能够连接到任何地方，包括 Telegram、Discord、WhatsApp、微信、QQ、钉钉、LINE、企业微信、飞书、Slack、IRC、OneBot、MQTT、MaixCam 等。
...
PicoClawQuant 使用统一的命令定义来源。启动时会自动将 Telegram 支持的命令（例如 `/start`、`/help`、`/show`、`/list`、`/use`、`/btw`）注册到 Bot 命令菜单，确保菜单展示与实际行为一致。
...
PicoClawQuant 支持两种 WhatsApp 连接方式：
...
PicoClawQuant 通过腾讯 iLink 官方 API 支持连接微信个人号。
...
PicoClawQuant 通过 WebSocket/SDK 模式连接飞书 — 无需公网 Webhook URL 或回调服务器。
...
PicoClawQuant 现在将企业微信统一为一个基于 WebSocket 的 AI Bot 渠道。
...
OneBot 是 QQ 机器人的开放协议。PicoClawQuant 通过 WebSocket 连接任何 OneBot v11 兼容实现（如 [Lagrange](https://github.com/LagrangeDev/Lagrange.Core)、[NapCat](https://github.com/NapNeko/NapCatQQ)）。
...
任意 MQTT 客户端均可通过 Broker 与 PicoClawQuant 通信。设备或服务向 Broker 发布请求，PicoClawQuant 订阅后处理并将响应发布回去。
...
```
{prefix}/{agent_id}/{client_id}/request    # 客户端 → PicoClawQuant
{prefix}/{agent_id}/{client_id}/response   # PicoClawQuant → 客户端
```
