...
| `provider` | string | No | Preferred provider identifier. When present, PicoClawQuant sends `model` unchanged to that provider                                                                                                                                              |
...
By default, PicoClawQuant now forwards tool JSON Schemas unchanged.
...
- Default behavior is disabled. If you omit `tool_schema_transform`, PicoClawQuant sends the original tool schema.
- The setting is per model entry, so you can enable it only for the providers that need it.

#### Provider / Model Resolution

PicoClawQuant resolves `provider` and the runtime model ID using these rules:
...
If `voice.model_name` is not configured, PicoClawQuant will continue to fall back to Groq transcription when a Groq API key is available.
...
- PicoClawQuant still uses the OpenAI-compatible `/audio/speech` route for this setup.
...
With explicit `provider`, PicoClawQuant sends `openai/gpt-oss-20b` unchanged to the LM Studio server. The legacy compatibility form `"model": "lmstudio/openai/gpt-oss-20b"` still resolves to the same upstream model ID when `provider` is omitted.
...
PicoClawQuant already supports automatic failover when you configure `primary` + `fallbacks` in the agent model settings.
...
If you use key-level failover for the same model, PicoClawQuant can chain through additional key-backed candidates before moving to cross-model backups.
...
<div align="center">
  <img src="../../assets/logo.jpg" alt="PicoClawQuant Meme" width="512">
</div>
