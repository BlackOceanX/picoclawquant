...
| **Pico**             | ⭐ Easy            | Native PicoClawQuant protocol channel                      |                                                                                                                  |
...
PicoClawQuant now keeps command definitions in one shared registry. On startup, Telegram will automatically register supported bot commands (for example `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) so command menu and runtime behavior stay in sync.
...
If command registration fails (network/API transient errors), the channel still starts and PicoClawQuant retries registration in the background.
...
PicoClawQuant can connect to WhatsApp in two ways:
...
PicoClawQuant supports connecting to your personal WeChat account using the official Tencent iLink API.
...
PicoClawQuant now exposes WeCom as a single AI Bot channel over WebSocket.
...
PicoClawQuant connects to Feishu via WebSocket/SDK mode — no public webhook URL or callback server needed.
...
OneBot is an open protocol for QQ bots. PicoClawQuant connects to any OneBot v11 compatible implementation (e.g., [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
...
Any MQTT client can communicate with PicoClawQuant via a broker. Devices or services publish requests to the broker; PicoClawQuant subscribes, processes them, and publishes responses back.
