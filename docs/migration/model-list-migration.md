...
1. If `provider` is set, PicoClawQuant sends `model` unchanged.
2. If `provider` is omitted, PicoClawQuant treats the first `/` segment in `model` as the provider and everything after that first `/` as the runtime model ID.
...
- [GitHub Issues](https://github.com/BlackOceanX/picoclawquant/issues)
- [Discussion #122](https://github.com/BlackOceanX/picoclawquant/discussions/122): Original proposal
