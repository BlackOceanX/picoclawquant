...
func (c *OpenClawConfig) ConvertToPicoClawQuant(sourceHome string) (*PicoClawQuantConfig, []string, error) {
	cfg := &PicoClawQuantConfig{}
	var warnings []string
	...
	if c.HasMemory() {
		warnings = append(warnings, "Memory backend config not migrated - PicoClawQuant uses SQLite with vector embeddings")
	}
	if c.HasCron() {
		warnings = append(
			warnings,
			"Cron job scheduling not supported in PicoClawQuant - consider using external schedulers",
		)
	}
	if c.HasHooks() {
		warnings = append(warnings, "Webhook hooks not supported in PicoClawQuant - use event system instead")
	}
	if c.HasSession() {
		warnings = append(warnings, "Session scope config differs - PicoClawQuant uses per-agent sessions by default")
	}
...
type PicoClawQuantConfig struct {
...
func (c *OpenClawConfig) convertChannels(warnings *[]string) ChannelsConfig {
	...
	if c.Channels.Signal != nil {
		*warnings = append(*warnings, "Channel 'signal': No PicoClawQuant adapter available")
	}
	if c.Channels.IRC != nil {
		*warnings = append(*warnings, "Channel 'irc': No PicoClawQuant adapter available")
	}
	if c.Channels.Mattermost != nil {
		*warnings = append(*warnings, "Channel 'mattermost': No PicoClawQuant adapter available")
	}
	...
	if c.Channels.BlueBubbles != nil {
		*warnings = append(
			*warnings,
			"Channel 'bluebubbles': No PicoClawQuant adapter available - consider iMessage instead",
		)
	}
...
func (c *PicoClawQuantConfig) ToStandardConfig() *config.Config {
