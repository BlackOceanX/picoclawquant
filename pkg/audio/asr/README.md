...
This package handles speech-to-text for PicoClawQuant voice input.
...
PicoClawQuant does not keep ASR API keys inside the `voice` section.
...
This is the recommended pattern because it is explicit, reusable, and consistent with the rest of PicoClawQuant's model configuration.
...
- You can omit `api_base` and PicoClawQuant will use Groq's default API base automatically.
...
PicoClawQuant currently supports three main ASR routes:
...
## How PicoClawQuant Chooses a Transcriber
...
2. If that resolved model is:
   - an `elevenlabs` provider model, PicoClawQuant uses the ElevenLabs transcriber.
   - an OpenAI-compatible Whisper model, PicoClawQuant uses the Whisper transcriber.
   - an audio-capable chat model, PicoClawQuant uses `AudioModelTranscriber`.
3. **Fallback path**: if `voice.model_name` is not set, PicoClawQuant performs a compatibility scan through `model_list` for legacy auto-detected ASR entries.
