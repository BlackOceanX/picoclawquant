...
func TestMentionsBot(t *testing.T) {
	tests := []struct {
		...
		{"multi word display name", "hey PicoClawQuant Bot, can you help", "PicoClawQuant Bot", "bot@example.org", true},
		...
		{"display name with prefix word", "hey SuperPicoClawQuant Bot", "PicoClawQuant Bot", "bot@example.org", false},
...
func TestEnsureAccountCreatesBootstrapAccountAndStops(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "bot123@mehl.cloud") || !strings.Contains(err.Error(), "run PicoClawQuant again") {
