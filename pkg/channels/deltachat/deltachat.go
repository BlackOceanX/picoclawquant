// Package deltachat implements a PicoClawQuant channel for Delta Chat, an
// email-based, end-to-end encrypted messenger.
//
// PicoClawQuant does not link the Delta Chat core directly. Instead it drives a
...
// wire) that PicoClawQuant sets when calling send_msg. Viewtype is normally left empty
...
func (c *DeltaChatChannel) createChatmailBootstrapAccount(ctx context.Context, server string) error {
	...
	return fmt.Errorf(
		"deltachat: created chatmail account %q on %s. Update channel_list.deltachat.settings.email from %q to %q, remove the @server bootstrap marker, then run PicoClawQuant again to use the account",
		addr,
		server,
		c.config.Email,
		addr,
