...
import (
	...
	pkgauth "github.com/BlackOceanX/picoclawquant/pkg/auth"
...
func setAuthStatusTestHome(t *testing.T) string {
	...
	tmpDir := t.TempDir()
	t.Setenv(config.EnvHome, filepath.Join(tmpDir, ".picoclawquant"))
	return tmpDir
}
...
func TestAuthStatusCmdShowsCanonicalGoogleAntigravityAfterLegacyRefresh(t *testing.T) {
	...
	authPath := filepath.Join(tmpDir, ".picoclawquant", "auth.json")
